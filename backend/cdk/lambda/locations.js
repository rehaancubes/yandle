...
function triggerSync(handle) {
  ...
  if (!arn) return;
  lambda.invoke({ FunctionName: arn, InvocationType: "Event", Payload: JSON.stringify({ handle }) }).promise().catch((e) => console.error("[locations] sync invoke", e));
}
...
exports.handler = async (event) => {
  try {
    ...
    if (!process.env.LOCATIONS_TABLE) {
    ...
    if (method === "GET") {
      ...
      return {
        ...
        body: JSON.stringify({ handle, locations: result.Items || [] })
      };
    }
    ...
    if (method === "POST") {
      ...
      return {
        statusCode: 201,
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ ok: true, location: item })
      };
    }
    ...
    if (method === "DELETE") {
      ...
      const locationId = q.locationId || event.pathParameters?.locationId || body.locationId;
      if (!handle || !locationId) {
        return { statusCode: 400, headers: { "content-type": "application/json" }, body: JSON.stringify({ error: "handle and locationId are required" }) };
      }
      await assertAccess(handle, sub, email);
      await ddb.delete({
        TableName: process.env.LOCATIONS_TABLE,
        Key: { handle, locationId: String(locationId) }
      }).promise();
      triggerSync(handle);
